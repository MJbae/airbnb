...
interface Props {
  onClickPeriod: (e: React.MouseEvent) => void;
  size: string;
}

function Period({ onClickPeriod, size }: Props) {
  ...
  const checkinText: string =
    checkinDate !== '날짜 입력'
      ? `${checkinDate.substr(4, 2)}월 ${checkinDate.substr(6, 2)}일`
      : checkinDate;
  const checkoutText: string =
    checkoutDate !== '날짜 입력'
      ? `${checkoutDate.substr(4, 2)}월 ${checkoutDate.substr(6, 2)}일`
      : checkoutDate;
  return (
    <>
      <CheckInContainer
        onClick={onClickPeriod}
        className="betweenBorder"
        size={size}
      >
        <Title>체크인</Title>
